      ...
      <SheetContent className="w-full sm:max-w-md overflow-y-auto bg-white text-zinc-900 border-zinc-200">
        <SheetHeader className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            {stage && (
              <Badge variant="outline" className="border-yellow-500/40 bg-yellow-500/10 text-yellow-800 gap-1">
        ...
        <div className="px-4 pb-6 mt-2">
          ...
          <ul className="divide-y divide-zinc-200 rounded-lg border border-zinc-200 bg-zinc-50/60">
            {dept.positions.map((p, i) => (
              <li key={i} className="flex items-start justify-between gap-3 px-3 py-2 text-sm">
                ...
                <span className="shrink-0 rounded-md bg-zinc-100 px-2 py-0.5 text-xs text-zinc-600">×{p.count}</span>
              </li>
            ))}
          </ul>
          ...
          <Button
            variant="outline"
            className="mt-5 w-full border-zinc-300 bg-zinc-100 text-zinc-900 hover:bg-zinc-200"
